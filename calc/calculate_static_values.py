...
def regression_results():
    ...
    results['regularization'] = {
        'coef': round(ridge.coef_.ravel()[0], 4)
    }
    return results
...
@lru_cache(maxsize=1)
def statistical_learning_results():
    results = {}
    ...
    model = LinearRegression()
    # Note: cross_val_score might vary slightly unless seeded perfectly inside, but np.random.seed(42) helps.
    ...
    # We need to set seed/state for reproducibility if we want exact same numbers every run, but 'frozen' values will be fine.
    np.random.seed(42) 
    for i in range(500):
...
@lru_cache(maxsize=1)
def probability_basics_results():
...
@lru_cache(maxsize=1)
def probability_distributions_results():
    # --- 1. Discrete ---
    # Bernoulli (p=0.6)
...
@lru_cache(maxsize=1)
def statistical_inference_results():
    # --- 1. Sampling Methods ---
...
if __name__ == "__main__":
    ...
    print("BAYESIAN:", bayesian_results())
    print("MULTIVARIATE:", multivariate_results())
    print("STAT_LEARNING:", statistical_learning_results())
    print("EXPERIMENTAL:", experimental_results())
    print("STATISTICS:", statistics_results())
    print("PROBABILITY BASICS:", probability_basics_results())
    print("PROBABILITY DISTRIBUTIONS:", probability_distributions_results())
    print("STATISTICAL INFERENCE:", statistical_inference_results())
